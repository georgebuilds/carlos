...
var builtinRows = []permsBuiltinRow{
	...
	{"todo_list", "read todos from the configured backends"},
	{"todo_add", "add a todo to a frame's backend (Obsidian vault by default)"},
	{"todo_done", "mark a todo complete by id"},
	{"todo_update", "edit a todo's text, due date, tags, or state"},
	{"carlos_about", "read-only introspection of carlos's own state"},
	{"skill_use", "read a loaded skill's body — pure read, no side effects"},
